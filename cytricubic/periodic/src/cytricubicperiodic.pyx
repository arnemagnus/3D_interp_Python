...
Here, periodic boundary conditions along all three abscissae is used.
...
cdef class TricubicPeriodicInterpolator:
    """
    TricubicPeriodicInterpolator(x, y, z, data)
    ...
    def __cinit__(self, double[::1] x not None, double[::1] y not None,
        ...
        self.dy = y[1]-y[0]
        self.dz = z[1]-z[0]
        # As periodic boundary conditions are used, we map the last index in
    ...
    def ev_grid(self, double[::1] x, double[::1] y, double[::1] z,
        ...
        # level
        """
        TricubicPeriodicInterpolator.ev_grid(x, y, z, kx, ky, kz)
    ...
    cdef _set_voxel_indices_(self, int ix, int *ixm1, int *ixp1, int *ixp2,
    ...
    cdef _set_vals_(self, int x, int xp1, int y, int yp1, int z, int zp1):
        cdef:
    ...
    @cython.wraparound(False)
    @cython.initializedcheck(False)
    cdef _set_derivs_(self, int x, int xm1, int xp1, int xp2,
    ...
    @cython.wraparound(False)
    @cython.initializedcheck(False)
    cdef _set_mxd_2derivs_(self, int x, int xm1, int xp1, int xp2,
                                 int y, int ym1, int yp1, int yp2,
                                 int z, int zm1, int zp1, int zp2):
        cdef:
            double *psi = &self.psi[0]
    ...
    @cython.wraparound(False)
    @cython.initializedcheck(False)
    cdef _set_mxd_3deriv_(self, int x, int xm1, int xp1, int xp2,
                                int y, int ym1, int yp1, int yp2,
                                int z, int zm1, int zp1, int zp2):
        cdef:
            double *psi = &self.psi[0]
