...
interpolation routine routine in 3D, subject to periodic boundary conditions
along all three abscissae.
"""
...
cdef class TrilinearPeriodicInterpolator:
    """
    TrilinearPeriodicInterpolator(x, y, z, data)

    Constructor for a TrilinearPeriodicInterpolator object. Intended for use on
    ...
    def __cinit__(self,double[::1] x not None,double[::1] y not None,
        ...
        self.dy = y[1]-y[0]
        self.dz = z[1]-z[0]
        # When periodic boundary conditions are used, we map the last index in
    ...
    def ev(self, double x, double y, double z):
        ...
        # C level
        """
        TrilinearPeriodicInterpolator.ev(x, y, z)

        Evaluate the interpolated function at a single point (x, y, z), subject
    ...
    cdef double _ev_(self, double x, double y, double z):
        ...
        self._set_voxel_indices_(&x, &y, &z, &ix, &ixp1,
                                                          &iy, &iyp1,
                                                          &iz, &izp1)
    ...
    def ev_grid(self, double[::1] x, double[::1] y, double[::1] z):
        ...
        # C level
        """
        TrilinearPeriodicInterpolator.ev_grid(x, y, z)

        Evaluate the interpolated function at the grid spanned by the input
        x, y and z arrays, subject to periodic boundary conditions
